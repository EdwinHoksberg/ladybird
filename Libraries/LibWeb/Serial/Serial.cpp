...
WebIDL::ExceptionOr<GC::Ref<WebIDL::Promise>> Serial::request_port(SerialPortRequestOptions const options)
{
    ...
    if (!relevant_global_object.has_transient_activation()) {
        return WebIDL::create_rejected_promise(realm, WebIDL::SecurityError::create(realm, "Failed to execute 'requestPort' on 'Serial': Must be handling a user gesture to show a permission request."_string));
    }
        ...
        for (auto const& filter : options.filters.value()) {
            ...
            if (filter.bluetooth_service_class_id.has_value()) {
                ...
                if (filter.usb_vendor_id.has_value()) {
                    return WebIDL::create_rejected_promise(realm, JS::TypeError::create(realm, "A filter cannot specify both bluetoothServiceClassId and usbVendorId or usbProductId."_string));
                }
            ...
            if (filter.usb_vendor_id.has_value()) {
                return WebIDL::create_rejected_promise(realm, JS::TypeError::create(realm, "A filter containing a usbProductId must also specify a usbVendorId."_string));
            }
        }
    }
    ...
    Platform::EventLoopPlugin::the().deferred_invoke(GC::create_function(realm.heap(), [&realm, &relevant_global_object, promise, options]() -> void {
        ...
        if (!selected_port.has_value()) {
            queue_global_task(HTML::Task::Source::SerialPort, realm.global_object(), GC::create_function(realm.heap(), [&realm, promise]() {
                ...
                WebIDL::reject_promise(realm, promise, WebIDL::NotFoundError::create(realm, "Failed to execute 'requestPort' on 'Serial': No port selected by the user."_string));
            }));
            return;
        }
